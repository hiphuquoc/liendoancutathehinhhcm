...
<div class="formBox">
    <div class="formBox_full">
        ...
        <div class="formBox_full_item">
            ...
            <input type="text" id="seo_title" class="form-control {{ !empty($flagCopySource)&&$flagCopySource==true ? 'inputSuccess' : '' }}" name="seo_title" value="{{ old('seo_title') ?? $itemSeo['seo_title'] ?? '' }}" {{ $chatgptDataAndEvent['dataChatgpt'] ?? null }} {{ $readonly }} required>
        ...
        <div class="formBox_full_item">
            ...
            @endphp
            <input type="text" id="slug" class="form-control" name="slug" value="{{ $slug }}" {{ $chatgptDataAndEvent['dataChatgpt'] ?? null }} {{ $readonly }} required>
            <div class="invalid-feedback">{{ config('message.admin.validate.not_empty') }}</div>
        </div>
        {{-- <!-- One Row -->
        @if(!empty($sources))
        ...
        @if(!empty($parents)&&$parents->isNotEmpty())
            <div class="formBox_full_item">
                ...
                <div class="{{ !empty($flagCopySource)&&$flagCopySource==true ? 'boxInputSuccess' : '' }}">
                    <select class="select2 form-select select2-hidden-accessible" id="parent" name="parent" {{ $readonly }}>
        ...
        <div class="formBox_full_item">
            <div class="flexBox">
                <div class="flexBox_item">
                    ...
                    <input type="number" id="rating_aggregate_count" class="form-control {{ !empty($flagCopySource)&&$flagCopySource==true ? 'inputSuccess' : '' }}" name="rating_aggregate_count" value="{{ old('rating_aggregate_count') ?? $itemSeo['rating_aggregate_count'] ?? $item->seo['rating_aggregate_count'] ?? rand(1000,10000) }}" {{ $readonly }} required>
                    <div class="invalid-feedback">{{ config('message.admin.validate.not_empty') }}</div>
                </div>
                <div class="flexBox_item" style="margin-left:1rem;">
                    ...
                    <input type="text" id="rating_aggregate_star" class="form-control {{ !empty($flagCopySource)&&$flagCopySource==true ? 'inputSuccess' : '' }}" name="rating_aggregate_star" value="{{ old('rating_aggregate_star') ?? $itemSeo['rating_aggregate_star'] ?? $item->seo['rating_aggregate_star'] ?? '4.'.rand(6,8) }}" {{ $readonly }} required>
                    <div class="invalid-feedback">{{ config('message.admin.validate.not_empty') }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="formBox_full_item">
            ...
            <input type="text" id="topic" class="form-control" name="topic" readonly>
        </div> --}}
    </div>
</div>
@push('scriptCustom')
