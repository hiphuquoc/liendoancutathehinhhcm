...
class AthleteManagementController extends Controller
{
    ...
    {
        return view('admin.athleteManagement.index');
    }
    ...
    /**
     * Tạo athlete_code từ tháng/năm
     */
    private function generateAthleteCode($month, $year, $orderNumber)
    {
    ...
    /**
     * Lấy số thứ tự tiếp theo cho athlete_code dựa trên tháng/năm
     * Tìm athlete_code có số thứ tự cao nhất trong cùng tháng/năm và trả về số tiếp theo
     */
    private function getNextOrderNumber($month, $year)
    {
        // Pattern để tìm athlete_code có dạng: N.O:XXX.T{month}.{year}/VDV-HWBF
        // Ví dụ: N.O:001.T01.26/VDV-HWBF
        $pattern = "%.T{$month}.{$year}/VDV-HWBF";
        
        // Tìm tất cả trainer có athlete_code khớp pattern
        $existingCodes = Athlete::where('athlete_code', 'LIKE', $pattern)
            ->pluck('athlete_code')
            ->toArray();
        ...
        // Trích xuất số thứ tự từ mỗi athlete_code và tìm max
        $maxOrderNumber = 0;
    ...
    public function uploadExcel(Request $request)
    {
        ...
        try {
            ...
            $parent = Seo::where('slug', config('main_' . env('APP_NAME') . '.slug_athlete_parent'))
                ->first();
            ...
            $athleteExample = Athlete::with('achievements', 'skills', 'experiences.contents', 'degrees.contents')
                ->orderBy('id')
                ->first();
            ...
            if ($athleteExample) {
                foreach ($athleteExample->achievements as $achi) {
                    if (!empty($achi->content)) {
                        $dataAchievements[] = ['content' => $achi->content];
                    }
                }

                foreach ($athleteExample->skills as $skill) {
                ...
                foreach ($athleteExample->experiences as $exper) {
                ...
                foreach ($athleteExample->degrees as $degree) {
            ...
            foreach ($trainersData as $trainer) {
                ...
                if (!empty($checkEmail) && filter_var($checkEmail, FILTER_VALIDATE_EMAIL)) {
                    $existing = Athlete::where('email', $checkEmail)->first();
                    if ($existing) {
                ...
                if (!empty($checkPhone)) {
                    $existing = Athlete::where('phone', $checkPhone)->first();
                    if ($existing) {
            ...
            foreach ($trainersData as $trainerData) {
                ...
                try {
                    ...
                    $seoData = [
                        ...
                        'slug' => $slug,
                        'repeater_athlete_achievement' => $dataAchievements,
                        'repeater_athlete_skill' => $dataSkills,
                        'repeater_athlete_experience' => $dataExperiences,
                        'repeater_athlete_degree' => $dataDegrees,
                    ];

                    // Tạo request object để sử dụng AthleteRequest validation
                    $trainerRequest = AthleteRequest::create(
                        route('admin.athlete.view'),
                        'POST',
                        $seoData
                    );
                    $trainerRequest->setLaravelSession(session());

                    // Gọi AthleteController để tạo trainer
                    $trainerController = app(\App\Http\Controllers\Admin\AthleteController::class);
                    $result = $trainerController->createAndUpdate($trainerRequest);

                    // Lấy trainer vừa tạo
                    $trainer = Athlete::whereHas('seo', function ($query) use ($slug) {
                        $query->where('slug', $slug);
                    })->first();

                    if ($trainer) {
                        // Cập nhật athlete_code
                        $trainer->athlete_code = $trainerCode;
                        $trainer->save();
                        ...
                        if ($existingUser) {
                            ...
                            $trainerUsingUser = \App\Models\Athlete::where('user_id', $existingUser->id)
                            ...
                            if ($trainerUsingUser) {
                                ...
                                $results[] = [
                                    ...
                                    'athlete_code' => null,
                                    'slug' => $slug,
                                    'reasons' => ['Email/Username đã tồn tại trong hệ thống (Vận động viên)'],
                                    'qr_code' => null,
                                ];
                                ...
                                $trainerRoleId = \App\Models\Role::where('slug', 'athlete')->value('id');
                                if ($trainerRoleId) {
                                ...
                                if (!$existingUser->hasRole('athlete')) {
                                    $existingUser->role = 'athlete';
                                    $existingUser->save();
                                }
            ...
            Log::error("AthleteManagement uploadExcel error: " . $e->getMessage());
