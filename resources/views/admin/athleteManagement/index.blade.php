...
@include('admin.components.loadingOverlay', [
    'id' => 'athleteManagementLoadingOverlay',
                ...
                <div class="companyManagementPage_section_header_left">
                    ...
                    <div class="companyManagementPage_section_header_info">
                        <h2 class="companyManagementPage_section_title">
                            Cập nhật VĐV từ Excel
                        </h2>
                        <p class="companyManagementPage_section_desc">Tải lên file Excel để tạo hàng loạt vận động viên và tài khoản tương ứng</p>
                    </div>
                </div>
            </div>
            <div class="companyManagementPage_section_body">
                <form id="athleteUploadForm" enctype="multipart/form-data" class="adminTrainerManagement_form">
                    @csrf
                    <div class="adminTrainerManagement_form_body">
                        ...
                        <div class="adminFormField_hint" style="margin-bottom: 1.5rem;">
                            <p style="font-size: 0.8125rem; color: var(--admin-gray-600); margin: 0.25rem 0;">Chọn tháng và năm để tạo mã VĐV cho khóa học này (ví dụ: Tháng 12, Năm 2025 → mã sẽ có dạng T12.25)</p>
                        </div>
...
(function() {
    ...
    function initUploadForm() {
        const form = document.getElementById('athleteUploadForm');
        if (!form) {
            console.error('Form athleteUploadForm not found');
            return;
        }
        ...
        form.addEventListener('submit', async function(e) {
            ...
            if (typeof showAdminLoading === 'function') {
                showAdminLoading('athleteManagementLoadingOverlay', 'Đang xử lý file Excel...', true);
            }
            ...
            const uploadForm = document.getElementById('athleteUploadForm');
            if (uploadForm) {
            ...
            if (typeof updateAdminLoadingProgress === 'function') {
                updateAdminLoadingProgress('athleteManagementLoadingOverlay', 10, 'Đang tải file lên server...');
            }
    
    try {
            const response = await fetch('{{ route("admin.athleteManagement.uploadExcel") }}', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            // Update progress
            if (typeof updateAdminLoadingProgress === 'function') {
                updateAdminLoadingProgress('athleteManagementLoadingOverlay', 30, 'Đang đọc file Excel...');
            }
            ...
            const progressInterval = setInterval(() => {
                if (progressPercent < 90) {
                    progressPercent += 3;
                    if (typeof updateAdminLoadingProgress === 'function') {
                        updateAdminLoadingProgress('athleteManagementLoadingOverlay', progressPercent, 'Đang xử lý dữ liệu...');
                    }
                }
            }, 200);
            ...
            if (typeof updateAdminLoadingProgress === 'function') {
                updateAdminLoadingProgress('athleteManagementLoadingOverlay', 100, 'Hoàn thành!');
            }
            
            // Hide loading overlay after a short delay
            setTimeout(() => {
                if (typeof hideAdminLoading === 'function') {
                    hideAdminLoading('athleteManagementLoadingOverlay');
                }
            }, 500);
                        ...
                        detailsHtml = `
                            <div class="adminQrCode_listItem_details">
                                <div class="adminQrCode_listItem_nameRow">
                                    ...
                                    ${result.athlete_code ? `<span class="adminQrCode_listItem_code">${result.athlete_code}</span>` : ''}
