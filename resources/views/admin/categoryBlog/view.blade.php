...
@section('content')
@php
    $titlePage = 'Thêm Chuyên mục Blog mới';
    $submit = 'admin.categoryBlog.createAndUpdate';
    if(!empty($type) && $type == 'edit'){
        $titlePage = 'Chỉnh sửa Chuyên mục Blog';
    }
    ...
    <div class="adminFormPage">
        <div class="adminFormPage_content">
            ...
            @include('admin.components.pageHeader', [
                'title' => $titlePage,
                'desc' => $type == 'edit' ? 'Chỉnh sửa chuyên mục blog' : 'Tạo chuyên mục blog mới',
                'icon' => '<path d="M3.375 19.5h17.25m-17.25 0a1.125 1.125 0 01-1.125-1.125M3.375 19.5h1.5C5.496 19.5 6 18.996 6 18.375m-3.75 0V5.625m0 12.75v-1.5c0-.621.504-1.125 1.125-1.125h18.375m-10.5-1.5v10.5m0-10.5c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v10.5m-6 0h6"/>',
                'backUrl' => route('admin.categoryBlog.list'),
                'backText' => 'Quay lại'
            ])
                    ...
                    <div class="adminFormSection">
                        ...
                        <div class="adminFormSection_body">
                            ...
                            @if($language == 'vi')
                                ...
                                @php
                                    $flagShowChecked = false;
                                    if(empty($item)){
                                        $flagShowChecked = !empty($itemSourceToCopy->flag_show) && ($itemSourceToCopy->flag_show == 1);
                                    } else {
                                        $flagShowChecked = !empty($item->flag_show) && ($item->flag_show == 1);
                                    }
                                @endphp
                                @include('admin.components.formField', [
                                    'label' => 'Cho phép hiển thị trong danh sách',
                                    'name' => 'flag_show',
                                    'type' => 'checkbox',
                                    'value' => $flagShowChecked,
                                    'checkboxLabel' => 'Cho phép hiển thị trong danh sách'
                                ])
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="adminFormSidebar">
                        <div class="adminFormSidebar_sticky">
                            ...
                            @include('admin.components.formActions', [
                                'backRoute' => 'admin.categoryBlog.list',
                                'viewUrl' => $viewUrl,
