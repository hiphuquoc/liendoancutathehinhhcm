...
class AjaxController extends Controller {
    ...
    public static function registryEmail(Request $request){
        ...
        $email                  = $request->get('registry_email') ?? null;
        $infoEmail              = RegistryEmail::select('*')
                                    ->where('email', $email)
                                    ->first();
        
        if(empty($infoEmail)) $infoEmail = RegistryEmail::insertItem(['email' => $request->get('registry_email')]);
        if(!empty($infoEmail)){
            $result['type']     = 'success';
    ...
    public static function requestContact(Request $request){
        if(!empty($request->get('phone'))){
            /* insert vào CSDL */
            RequestContact::insertItem([
                'full_name' => $request->get('full_name'),
                'phone'     => $request->get('phone'),
                'message'   => $request->get('message'),
            ]);
            $result['type']     = 'success';
            $result['title']    = 'Gửi yêu cầu liên hệ thành công!';
            $result['content']  = '<div>Cảm ơn bạn đã quan tâm hoạt động của '.config('main_'.env('APP_NAME').'.info.'.env('APP_NAME').'.company_name').'!</div>
                                    <div>Quản trị viên sẽ liên hệ lại với bạn trong thời gian sớm nhất có thể, vui lòng chờ chúng tôi nhé.</div>
                                    <div>Chúc bạn có một ngày tuyệt vời!</div>'; 
            return json_encode($result);
        }
        $result['type']     = 'error';
        $result['title']    = 'Có lỗi xảy ra!';
        $result['content']  = '<div>Đã có lỗi không mong muốn xảy ra trong quá trình gửi yêu cầu, nếu cần thông tin gấp bạn vui lòng liên hệ trực tiếp với chúng tôi qua điện thoại '.config('main_'.env('APP_NAME').'.info.'.env('APP_NAME').'.hotline').'</div>'; 
        return json_encode($result);
    }

    public static function setMessageModal(Request $request){
        $response   = view('wallpaper.modal.contentMessageModal', [
